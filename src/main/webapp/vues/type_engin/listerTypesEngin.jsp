<%@page import="java.util.ArrayList, bts.sio.codecafe.model.TypeEngin"%>
...
<div class="card border-0">
    <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
            <h1 class="h3 mt-2 mb-0 fw-semibold">Liste des types d'engin</h1>
            <span class="text-secondary small mt-1">Liste des types d'engin avec action possible.</span>
        </div>
        ...
        <div class="btn-group" role="group">
            ...
            <a href="/26CodeCafe/ServletTypeEngin/lister?archive=0"
               class="btn btn-outline-primary <%= "0".equals(request.getParameter("archive")) ? "active" : "" %>">
                Non archivées
            </a>
            <a href="/26CodeCafe/ServletTypeEngin/lister?archive=1"
    ...
    <div class="card-body p-4">
        ...
        <table class="table table-striped table-hover" id="table">
            ...
            <tbody>
            <%
                ArrayList<TypeEngin> lesTypesEngin = (ArrayList<TypeEngin>) request.getAttribute("pLesTypesEngin");
                if (lesTypesEngin != null) {
                    for (TypeEngin s : lesTypesEngin) {
            %>
            <tr>
                <th scope="row"><%= s.getId() %></th>
                <td><%= s.getLibelle() %></td>
                <td>
                    <a href="${pageContext.request.contextPath}/ServletTypeEngin/consulter?idTypeEngin=<%= s.getId() %>"
                       data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Consulter" class="mx-1 text-decoration-none">
                        <i class="bi bi-eye"></i>
                    </a>
                    <a href="${pageContext.request.contextPath}/ServletTypeEngin/modifier?idTypeEngin=<%= s.getId() %>"
                    ...
                    <a href="${pageContext.request.contextPath}/ServletTypeEngin/archiver?idTypeEngin=<%= s.getId() %>&archive=<%= nouvelEtat %>&retour=<%= filtreActuel %>"
            ...
            <tr>
                <td colspan="4">
                    <%
                        String alertMessage = "Aucun type d'engin trouvée";
